...
public class BatchServer {
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            System.err.println("Usage: BatchServer <variant(asap,ondemand)>");
            System.exit(1);
        }
        boolean precompute = args.length > 0 && args[0].equalsIgnoreCase("asap");

        GraphEngine engine = new GraphEngine(null, "log/server_logs.csv", true);
        engine.setPrecomputedMode(precompute);
        engine.setSimulatedDelayMs(10);
    
        BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
        String line;

        while ((line = in.readLine()) != null) {
            if (line.trim().equals("S")) break;
